...
function updateAreaData( that, status, e){
    ...
    var getProvinceData = function (){
      ...
      countys.some((item, index) => {
        if (item.name.indexOf("吴中区") != -1) {
          vali[2] = index;
          return;
        }
      })
      getStreetInfo(vali[0], vali[1],vali[2]);
  ...
  var getStreetInfo = function (column0 = 0, column1 = 0, column2=0) {
    var c;
    streets = [];
    var num = 0;
    for (var i = 0; i < areaInfo.length; i++) {
      c = areaInfo[i];
      if (c.zhen != "00" && c.sheng == provinces[column0].sheng && c.di == citys[column1].di && c.xian == countys[column2].xian) {
        streets[num] = c;
        num++;
      }
    }
    ...
    value = [column0, column1, column2,0];
  }
    ...
    var valueChange = function(e,that){
      ...
      } else if(value[1] != val[1]) {    //若省份column未做滑动，地级市做了滑动则定位区县第一位
          val[2] = 0;
          val[3] = 0;
          getCountyInfo(val[0], val[1]);//获取区县数据
        
      } else if (value[2] != val[2]) {    //若省份column未做滑动，地级市做了滑动则定位区县第一位
       
        val[3] = 0;
        getStreetInfo(val[0], val[1],val[2]);//获取区县数据
